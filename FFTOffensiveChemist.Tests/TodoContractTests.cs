...
namespace FFTOffensiveChemist.Tests
{
    ...
    /// FFTLivingWeapons TodoContractTests pattern with the OC-n id prefix. RepoRoot walks up
    /// from the test bin dir to the first dir holding both docs/TODO.md and data/grenades.json.
    ///
    ...
    /// F. Every OC-id captured out of TODO.md's entries and CHANGELOG.md's entries is unique across
    ...
    public class TodoContractTests
    {
        ...
        private static readonly Regex NowEntryRegex = new(
            @"^- \*\*\[OC-(\d+)\] [^*]+\*\* \(opened \d{4}-\d{2}-\d{2}\) \[(QUEUED|BUILDING|AWAITING-LIVE|BLOCKED\([^)]+\))\]$",
            RegexOptions.Compiled);

        private static readonly Regex BacklogEntryRegex = new(
            @"^- \[OC-(\d+)\] \d{4}-\d{2}-\d{2}: .+", RegexOptions.Compiled);

        private static readonly Regex ChangelogEntryRegex = new(
            @"^- \[OC-(\d+)\] (SHIPPED [0-9a-f]{7,40} |WONTFIX |RETRACTED )\d{4}-\d{2}-\d{2}: .+",
            RegexOptions.Compiled);
        ...
        /// opening bracket (say "- OC-9 SHIPPED abc1234 2026-07-21: text") dodge both the grammar
        ...
        [Theory]
        [InlineData("- **[OC-1] Some title here** (opened 2026-07-21) [QUEUED]", true)]
        [InlineData("- **[OC-2] Some title here** (opened 2026-07-21) [BUILDING]", true)]
        [InlineData("- **[OC-3] Some title here** (opened 2026-07-21) [AWAITING-LIVE]", true)]
        [InlineData("- **[OC-4] Some title here** (opened 2026-07-21) [BLOCKED(owner live session)]", true)]
        [InlineData("- **[OC-5] Some title here** (opened 2026-07-21)", false)]                          // missing status token
        [InlineData("- **[OC-6] Some title here** (opened 2026-07-21) [WAITING]", false)]                 // unknown status token
        [InlineData("- **[OC-7] Some title here** (opened 2026-07-21) [BLOCKED]", false)]                 // BLOCKED needs a reason
        [InlineData("- **[OC-8] Some title here** (opened 2026-7-21) [QUEUED]", false)]                   // bad date shape
        [InlineData("- [OC-9] Some title here (opened 2026-07-21) [QUEUED]", false)]                      // missing bold markers
        [InlineData("- **[OC-10] Title** rogue **[OC-11] second marker** (opened 2026-07-21) [QUEUED]", false)]  // greedy title must not swallow a second "**[" marker
        ...
        [Theory]
        [InlineData("- [OC-10] 2026-07-21: One sentence description.", true)]
        [InlineData("- [OC-11] 2026-07-21: ", false)]                          // no sentence text after the colon
        [InlineData("- [OC-12] 2026-7-21: One sentence description.", false)]   // bad date shape
        [InlineData("- **[OC-13] Some title** 2026-07-21: text", false)]       // bold markers not allowed here
        [InlineData("- OC-14 2026-07-21: text", false)]                        // missing the [OC-n] brackets
        ...
        [Theory]
        [InlineData("- [OC-15] SHIPPED 58d5c7b 2026-07-21: summary text.", true)]
        [InlineData("- [OC-16] WONTFIX 2026-07-21: summary text.", true)]
        [InlineData("- [OC-17] RETRACTED 2026-07-21: summary text.", true)]
        [InlineData("- [OC-18] SHIPPED 2026-07-21: summary text.", false)]           // SHIPPED without a hash
        [InlineData("- [OC-19] SHIPPED zzzzzzz 2026-07-21: summary text.", false)]   // hash not hex
        [InlineData("- [OC-20] DEFERRED 2026-07-21: summary text.", false)]         // unknown disposition
        [InlineData("- [OC-21] SHIPPED 58d5c7b 2026-7-21: summary text.", false)]    // bad date shape
        [InlineData("- OC-22 SHIPPED abc1234 2026-07-21: summary text.", false)]     // missing the [OC-n] brackets
        ...
        {
            // Mirrors the real docs/CHANGELOG.md shape: a header/preamble, a "## ... " section
            ...
            var lines = new[]
            {
                ...
                "## 1.2.0 cycle",
                "",
                "- [OC-1] SHIPPED abc1234 2026-07-21: a well-formed entry.",
                "  continuation prose for OC-1.",
                "- OC-9 SHIPPED abc1234 2026-07-21: mangled entry missing its opening bracket.",
            };

            var scanned = ChangelogEntryLines(lines);

            Assert.Contains(scanned, l => l.Contains("OC-9"));
            string mangled = scanned.Single(l => l.Contains("OC-9"));
        ...
        [Fact]
        public void Every_OC_id_is_unique_across_TODO_and_CHANGELOG()
        {
            string repoRoot = RepoRoot();
            ...
            Assert.True(duplicates.Count == 0,
                "Duplicate OC-ids found:\n" + string.Join("\n",
                    duplicates.Select(g => $"OC-{g.Key}: " + string.Join(" | ", g.Select(x => x.Source)))));
        }
        ...
        // The em dash CHARACTER lives here as a \u escape, not a literal: the repo bans the
        // literal glyph in committed files, and this test is what enforces the ban.
